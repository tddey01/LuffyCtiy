...
from Shopping.views.ShoppingCar import SHOPPINGCAR_KEY
import redis
import json
...
class SettlementView(APIView):
    ...
    def post(self, request):
        ...
        # 1 获取前端的数据以及user_id
        coures_list = request.data.get('coures_list', "")
        user_id = request.user.pk
        # 2 校验数据的合法性
        for course_id in coures_list:
            # 2.1 判断coures_id 是否在购物车中
            ...
            # 3 构建数据结构
            # 3.1 获取用户所有的所有合法优惠券
            user_all_coupons = models.CouponRecord.objects.filter(
                account__id=user_id,
                status=0,
            ...
            for coupon_record in user_all_coupons:
                ...
                if coupon_record.object_id == course_id:
